...
print(xtrain)
ytrain=df.opinion[:1600]
print(ytrain)
tmp2=df.opinion[2400:]
ytrain=pd.concat([ytrain,tmp2],ignore_index=True)
xtest=df.cleaned[1600:2400]
ytest=df.opinion[1600:2400]



X_train, X_test, y_train, y_test = train_test_split(df['cleaned'], df.opinion, test_size=0.2)

count = CountVectorizer(ngram_range=(1,2),analyzer="word",min_df=10,max_df=0.9)
...
xtrain=pd.concat([xtrain,tmp],ignore_index=True)
print(xtrain)
ytrain=df.opinion[:4000]
print(ytrain)
tmp2=df.opinion[6125:]
ytrain=pd.concat([ytrain,tmp2],ignore_index=True)
xtest=df.cleaned[4000:6000]
ytest=df.opinion[4000:6000]
